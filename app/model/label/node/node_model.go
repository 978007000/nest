...
// arModel is a active record design model for table label operations.
type arModel struct {
	gmvc.M
}

var (
	// Table is the table name of label.
	Table = "node"
	// Model is the model object of label.
	Model = &arModel{g.DB("default").Table(Table).Safe()}
	// Columns defines and stores column names for table label.
	Columns = struct {
		Id         string //           
        ParentId   string // 父级编号  
        Level      string // 节点级别  
        Name       string // 后台标签  
        ParentIds  string // 父级路径  
        IsValid    string // 是否有效  
        Sort       string // 同级排序  
        Txt        string // 描述文本
	}{
