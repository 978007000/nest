...
// arModel is a active record design model for table channel operations.
type arModel struct {
	gmvc.M
}

var (
	// Table is the table name of channel.
	Table = "channel"
	// Model is the model object of channel.
	Model = &arModel{g.DB("default").Table(Table).Safe()}
	// Columns defines and stores column names for table channel.
	Columns = struct {
		Id         string //                     
        Name       string // 栏目名称            
        TagIds     string // 前台标签聚合关系    
        IsValid    string // 状态，1启用，0禁用  
        CreatedAt  string //                     
        UpdatedAt  string //                     
        Sort       string // 同级排序
	}{
...
// Master marks the following operation on master label.
func (m *arModel) Master() *arModel {
	return &arModel{m.M.Master()}
}

// Slave marks the following operation on slave label.
// Note that it makes sense only if there's any slave label configured.
